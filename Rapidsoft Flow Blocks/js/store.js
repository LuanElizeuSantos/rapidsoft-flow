...
const FlowStore = {
  STORAGE_KEY: 'rapidsoft-flow-blocks-config',
  ...
  /** Dev (`npm run dev`): salva em data/rapidsoft-flow-blocks-config.json para Git. */
  ...
  async persistirArquivoProjeto() {
    ...
    try {
      ...
      console.warn('Não foi possível gravar data/rapidsoft-flow-blocks-config.json:', err);
      return false;
    }
  },
  ...
  getBlocklyWorkspaceProcessoAtivo() {
    if (!this.processoAtivo) return null;
    ...
    return pd?.blocklyWorkspace || null;
  },

  salvarBlocklyWorkspaceProcessoAtivo(json) {
    if (!this.processoAtivo) return;
    const pd = this.getProcessoDetalhadoDef(
      this.grupoAtivo,
      this.fluxoAtivo,
      this.processoAtivo,
