...
export default function App() {
  ...
  useEffect(() => {
    ...
    window.addEventListener('rapidsoft-flow-change', handler);
    return () => window.removeEventListener('rapidsoft-flow-change', handler);
  }, []);
  ...
  useEffect(() => {
    ...
    if (btnManutencao) {
      btnManutencao.hidden = emDetalhe;
    }
  }, [emDetalhe]);
  ...
  const voltarMacro = () => {
    ...
    setProcessoId(null);
    setRefreshKey((k) => k + 1);
    window.dispatchEvent(new Event('rapidsoft-flow-change'));
  };
    ...
    <>
      ...
      <div className="fluxo-info">
        ...
        <div className="legenda">
          ...
          {emDetalhe && (
            <>
              <span className="legenda__item"><span className="legenda__icon legenda__icon--processo" /> Ação RPA</span>
              <span className="legenda__item"><span className="legenda__icon legenda__icon--decisao" /> Controle</span>
              <span className="legenda__item"><span className="legenda__icon legenda__icon--cliente" /> Início</span>
            </>
          )}
        </div>
        {!semFluxo && (
          <p className="fluxo-info__dica-diagrama">
            {emDetalhe
              ? 'Monte o passo a passo com blocos — arraste ações, repetições e exporte Python para o RPA.'
              : 'Clique em uma etapa para detalhar · Arraste das bolinhas para criar setas · Baixar PNG ou Mermaid no canto'}
          </p>
        )}
      </div>

      <section className={`diagrama ${emDetalhe ? 'diagrama--blockly diagrama--react' : 'diagrama--react'}`} aria-live="polite">
        {semFluxo ? (
          <p className="flow__vazio">Nenhum fluxo cadastrado para este grupo.</p>
        ) : emDetalhe && processoId ? (
          <BlocklyWorkspace
            key={`blocks-${processoId}-${refreshKey}`}
